...
import { FaBath, FaShareAlt, FaBed, FaUserCircle, FaPlus, FaMapMarkerAlt, FaCamera, FaCameraRetro, FaHeart } from "react-icons/fa"
...
const Properties = () => {
  ...
  return (
    <div className={`${darkMode ? 'dark bg-black' : 'light bg-transparent'}`}>
      <section id='properties' className='lg:w-[90%] m-auto lg:px-20 px-6 py-20 w-full flex-col justify-center items-start gap-10'>
        ...
        {/* properties grid stars from here */}
        ...
        <div id='grid-box' className='w-full grid lg:grid-cols-3 grid-cols-1 justify-center items-center gap-8'>
          {
            property.map((item, index) => (
              <div data-aos='zoom-in' data-aos-delay='200' key={index} className='bg-white dark:bg-gray-800 rounded-xl w-full'>
                <div id='image-box' className='bg-cover bg-center h-[250px] rounded-xl p-4 flex flex-col justify-between items-end' style={{ backgroundImage: `url(${item.image})` }}>
                ...
                <div className='px-6 py-3 flex flex-col justify-center items-start gap-2 w-full'>
                  <h1 className='text-x1 text-balck font-semibold dark:text-white'>{item.name}</h1>
                  ...
                  <div className='w-full h-[1px] bg-gray-200 m-8'></div>
